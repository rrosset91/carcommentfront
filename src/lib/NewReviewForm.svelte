<script>
  ...
  let avgScore = (comfortRating+repairCostRating+fuelConsumptionRating+reliabilityRating+performanceRating)/totalConcepts;
  ...
  function handleCharactersLeft(e){
	if(e.target.id == "tips")
		tipsCharsLeft = 255 - e.target.value.length;
	else
	commmentCharsLeft = 255 - e.target.value.length;
  }
</script>
<main class="container">
	<form on:submit={submitForm}>
	  {#if avaliationPage == 1}
		<div class="form-group">
		  ...
		  <textarea on:keyup={handleCharactersLeft} class="form-control" id="comment" bind:value={comment} rows="3" maxlength="255"></textarea>
		  <p class="charsleft">{commmentCharsLeft} caracteres restantes</p>
		</div>
  
		<div class="form-group">
		  <label for="tips">Dicas a possíveis compradores:</label>
		  <textarea on:keyup={handleCharactersLeft} class="form-control" id="tips" bind:value={tipsForBuyers} rows="3" maxlength="255"></textarea>
		  <p class="charsleft">{tipsCharsLeft} caracteres restantes</p>
		</div>
		<div class="row">
			<div class="form-group col-6">
				<label for="version">Versão</label>
				<input type="text" placeholder="Ex: 1.6 CDI Turbo" class="form-control" id="version" bind:value={version} maxlength="100">
			  </div>
			  <div class="form-group col-6">
	  ...
	  {#if avaliationPage == 2}
		<div class="row">
		  ...
		  <div class="col-md-2">
			<div class="form-group">
			  <label for="fuelConsumptionRating">{fuel=='Eléctrico' ? 'Autonomia':'Consumo'}</label>
		  ...
		  <div class="col-md-2">
			<div class="form-group">
			  ...
			  <input required type="number" class="form-control" id="reliabilityRating" bind:value={reliabilityRating} min="0" max="10">
			</div>
		  </div>
		  <div class="col-md-1">
		  </div>
  ...
  <div class="review-overview">
    ...
    <button on:click={() => avaliationPage--} class="btn btn-outline-primary">Voltar</button>
    <button type="submit" disabled={!submitReviewEnabled} class="btn btn-primary">Continuar</button>
  </div>
  {/if}
</form>
